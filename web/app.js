(() => {
  ...
  function draw(){
    ...
    const candles=visibleCandles();if(!candles.length)return;const pad={top:18,right:63,bottom:31,left:8},w=rect.width-pad.left-pad.right,h=rect.height-pad.top-pad.bottom,t=thresholds();let min=Math.min(...candles.map(c=>c.low),t?.lower??Infinity,t?.exitLower??Infinity),max=Math.max(...candles.map(c=>c.high),t?.upper??-Infinity,t?.exitUpper??-Infinity);const margin=Math.max((max-min)*.08,1);min-=margin;max+=margin;const y=v=>pad.top+(max-v)/(max-min)*h,x=i=>pad.left+(i+.5)/candles.length*w;
