(() => {
  ...
  const refs = {
    ...
    volumeALegend: $('#volume-a-legend'), volumeBLegend: $('#volume-b-legend')
  };
  ...
  function bind(){
    ...
    [refs.leftVenue,refs.rightVenue].forEach((select,index)=>select.addEventListener('change',()=>{const side=index?'right':'left',sourceSide=index?'left':'right',market=index?refs.rightMarket:refs.leftMarket;market.value='';marketSearch[side].selected=null;closeMarketMenu(side);renderIntervals();suggestEquivalent(sourceSide,side)}));
  ...
  function bindMarketPicker(side){
    ...
    input.addEventListener('input',()=>{search.selected=null;clearTimeout(search.timer);search.timer=setTimeout(()=>loadMarkets(side,true,input.value.trim()),140)});
