(() => {
  ...
  function draw(){
    ...
    const candles=visibleCandles();if(!candles.length)return;const pad={top:18,right:63,bottom:31,left:8},w=rect.width-pad.left-pad.right,h=rect.height-pad.top-pad.bottom,t=thresholds(),hasVolume=candles.some(candle=>Number.isFinite(candle.left_volume)||Number.isFinite(candle.right_volume)),volumeHeight=hasVolume?Math.min(82,h*.22):0,volumeGap=hasVolume?14:0,priceHeight=h-volumeHeight-volumeGap;let min=Math.min(...candles.map(c=>c.low),t?.lower??Infinity,t?.exitLower??Infinity),max=Math.max(...candles.map(c=>c.high),t?.upper??-Infinity,t?.exitUpper??-Infinity);const margin=Math.max((max-min)*.08,1);min-=margin;max+=margin;const y=v=>pad.top+(max-v)/(max-min)*priceHeight,x=i=>pad.left+(i+.5)/candles.length*w;
    ...
    if(hasVolume){const volumeTop=pad.top+priceHeight+volumeGap,volumeBottom=volumeTop+volumeHeight,maxLeft=Math.max(1,...candles.map(candle=>candle.left_volume).filter(Number.isFinite)),maxRight=Math.max(1,...candles.map(candle=>candle.right_volume).filter(Number.isFinite)),barWidth=Math.max(1,Math.min(step*.34,5));ctx.strokeStyle='#242b35';ctx.beginPath();ctx.moveTo(pad.left,volumeTop-.5);ctx.lineTo(pad.left+w,volumeTop-.5);ctx.stroke();candles.forEach((candle,index)=>{const xx=x(index),leftVolume=Number.isFinite(candle.left_volume)?candle.left_volume:0,rightVolume=Number.isFinite(candle.right_volume)?candle.right_volume:0,leftHeight=leftVolume/maxLeft*volumeHeight,rightHeight=rightVolume/maxRight*volumeHeight;if(leftVolume>0){ctx.fillStyle='rgba(107,140,255,.58)';ctx.fillRect(xx-barWidth,volumeBottom-leftHeight,barWidth,leftHeight)}if(rightVolume>0){ctx.fillStyle='rgba(167,132,255,.58)';ctx.fillRect(xx,volumeBottom-rightHeight,barWidth,rightHeight)}})}
