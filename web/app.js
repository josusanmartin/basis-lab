(() => {
  ...
  async function suggestEquivalent(sourceSide,targetSide){
    ...
    try {const q=new URLSearchParams({source_venue:source.select.value,source_symbol:source.input.value.trim(),target_venue:target.select.value,limit:'8'}),response=await fetchApi(`${apiBase}/api/v1/tickers/suggest?${q}`,{signal:target.search.abort.signal,headers:{Accept:'application/json'}});if(!response.ok)throw new Error();const data=(await response.json()).data;if(sequence!==target.search.sequence)return;if(!data.length){marketMessage(target.list,'No equivalent ticker found');return}renderMarketOptions(targetSide,data);openMarketMenu(targetSide)}
    catch(error){if(error.name==='AbortError')return;if(sequence===target.search.sequence)loadMarkets(targetSide,true,target.input.value.trim())}
  }
